...
public interface TransaccionRepository extends JpaRepository<Transaccion, Long> {
    ...
    @Query("SELECT COALESCE(SUM(t.monto), 0) FROM Transaccion t WHERE t.usuarioId = :usuarioId AND t.categoria IN ('Vivienda', 'Servicios') AND t.fecha BETWEEN :desde AND :hasta")
